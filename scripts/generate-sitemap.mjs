...
async function cacheProductImage(product, imageDirectory) {
  ...
  if (!filePath || !(await fileExists(filePath))) {
    ...
    try {
      ...
      // A foto remota pode bloquear downloads. Preserve uma cópia local da
      // mesma análise (inclusive de um ID duplicado) antes de usar o padrão.
      const ids = [product.id, ...[...productAliasTargets]
        .filter(([, canonical]) => canonical.id === product.id)
        .map(([aliasId]) => aliasId)];
      const cached = (await readdir(imageDirectory)).sort().find((name) =>
        ids.some((id) => name.startsWith(`${id}-`)) && /\.(?:jpe?g|png|webp)$/i.test(name));
      if (cached) {
        const cachedExtension = cached.split(".").pop().toLowerCase().replace("jpeg", "jpg");
        return {
          url: `${SITE}produto/imagens/${cached}?v=${SHARE_VERSION}`,
          mime: imageMime(cachedExtension),
          fileName: cached,
        };
      }
      return { url: fallback, mime: "image/png", fileName: "" };
...
function productMarketplace(product) {

  if (String(product?.marketplace || "").toLowerCase() === "shopee") return "shopee";
...
function weeklyTopSnapshot(product, image, position) {
  ...
  return {
    ...
    // A data resumida serve à seleção; o horário completo comprova a
...
function renderSharePage(product, socialImage, categoryNames, marketplaceStatus = null) {
  ...
  const apiCheckedAt = Date.parse(marketplaceStatus?.checkedAt || "");
  const apiConfirmed = marketplaceStatus?.managed === true && marketplaceStatus?.status === "active"
    && marketplaceStatus?.available === true && Number(marketplaceStatus?.price) > 0
    && Number.isFinite(apiCheckedAt) && Date.now() - apiCheckedAt >= 0
    && Date.now() - apiCheckedAt <= 24 * 60 * 60 * 1000;
  const manualCheckedAt = Date.parse(product.precoAtualizadoManualmenteEm || "");
  const lightningValid = product.ofertaRelampagoAtiva === true
    && Date.parse(product.ofertaRelampagoTerminaEm || "") > Date.now()
    && numberPrice(product.precoAnterior) > numberPrice(product.precoPromocional);
  const manualPrice = numberPrice(lightningValid || promotionIsValid(product)
    ? product.precoPromocional : product.preco);
  const manualConfirmed = product.precoAtualizadoManualmente === true && manualPrice > 0
    && Number.isFinite(manualCheckedAt) && Date.now() - manualCheckedAt >= 0
    && Date.now() - manualCheckedAt <= 24 * 60 * 60 * 1000;
  const useManual = manualConfirmed && (!apiConfirmed || manualCheckedAt >= apiCheckedAt);
  const confirmed = manualConfirmed || apiConfirmed;
  const chosenCheckedAt = useManual ? manualCheckedAt : apiCheckedAt;
  const priceDate = Number.isFinite(chosenCheckedAt)
    ? new Intl.DateTimeFormat("pt-BR", { timeZone: "America/Sao_Paulo" }).format(chosenCheckedAt) : "";
  const promotional = confirmed && promotionIsValid(product);
  const currentPrice = useManual ? manualPrice : numberPrice(marketplaceStatus?.price);
...
<script defer src="/seo-priorities.js?v=20260913-1"></script><script defer src="/growth-tools.js?v=20260923-audit-fix1"></script>
...
<body>
  ...
  ${offerUrl !== "#" ? `<a class="mobile-buy" id="mobile-affiliate-offer" href="${escapeHtml(offerUrl)}" target="_blank" rel="sponsored noopener noreferrer">Ver preço na ${escapeHtml(marketplaceName)}</a>` : ""}
  ...
  <script>document.getElementById('affiliate-offer')?.addEventListener('click',function(){gtag('event','select_item',{item_list_name:'pagina_produto',items:[{item_id:${JSON.stringify(product.id)},item_name:${JSON.stringify(title)}}],affiliate:${JSON.stringify(marketplace)}})});document.getElementById('mobile-affiliate-offer')?.addEventListener('click',function(){gtag('event','select_item',{item_list_name:'pagina_produto_mobile',items:[{item_id:${JSON.stringify(product.id)},item_name:${JSON.stringify(title)}}],affiliate:${JSON.stringify(marketplace)}})});document.getElementById('affiliate-offer-shopee')?.addEventListener('click',function(){gtag('event','select_item',{item_list_name:'pagina_produto',items:[{item_id:${JSON.stringify(product.id)},item_name:${JSON.stringify(title)}}],affiliate:'shopee'})});document.getElementById('share-product')?.addEventListener('click',async function(){const status=document.getElementById('share-status'),data={title:${JSON.stringify(title)},text:${JSON.stringify(`Confira esta análise no Ranking da Compra: ${title}`)},url:${JSON.stringify(shareUrl)}};try{if(navigator.share)await navigator.share(data);else{await navigator.clipboard.writeText(data.text+'\\n'+data.url);status.textContent='Texto e link copiados!'}gtag('event','share',{method:navigator.share?'system':'copy',content_type:'product',item_id:${JSON.stringify(product.id)}})}catch(error){if(error?.name!=='AbortError')status.textContent='Não foi possível compartilhar agora.'}});</script>

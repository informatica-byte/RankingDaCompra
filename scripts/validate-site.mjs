...
has(dashboardHtml, /id="admin-content"[\s\S]{0,350}id="consumo-titulo"/, "medidor privado da cota ausente", "dashboard.html");
has(dashboardHtml, /id="consumo-ponteiro"/, "ponteiro do consumo ausente", "dashboard.html");
has(dashboardHtml, /firebase-consumo\.js\?v=20260924-manual1/, "script do medidor privado ausente", "dashboard.html");
has(dashboardHtml, /id="consumo-leituras-informadas"[\s\S]*id="btn-consumo-salvar"/, "registro manual de leituras ausente", "dashboard.html");
has(firebaseGauge, /pacificDate\(at\) !== snapshot\.day/, "registro não valida o dia da cota", "firebase-consumo.js");
has(firebaseGauge, /root\.localStorage\.setItem\(STORAGE_KEY, JSON\.stringify\(snapshot\)\)/, "registro local ausente", "firebase-consumo.js");
has(firebaseGauge, /setAttribute\("transform", `rotate\(\$\{state\.angle\} 120 119\)`\)/, "ponteiro não gira no centro do medidor", "firebase-consumo.js");
if (/db\.collection\(|firebase\.firestore\(|monitoring\.googleapis\.com|requestAccessToken|fetch\(/i.test(firebaseGauge)) fail("firebase-consumo.js: medidor gratuito não pode consultar Firestore nem API paga");
...
for (const url of urls) {
  ...
  has(html, /<meta\s+property="og:image"\s+content="https:\/\/rankingdacompra\.com\.br\/(?:produto\/imagens\/[^"]+|og-ranking-da-compra\.png)"/i, "imagem social local ou imagem padrão ausente", relative);

...
has(dashboardHtml, /firebase-consumo\.js\?v=20260923-1/, "script do medidor privado ausente", "dashboard.html");
has(firebaseGauge, /firestore\.googleapis\.com\/document\/read_ops_count/, "fonte oficial de leituras ausente", "firebase-consumo.js");
has(firebaseGauge, /monitoring\.googleapis\.com\/v3\/projects/, "consulta privada ao Monitoring ausente", "firebase-consumo.js");
...
has(firebaseGauge, /btn-consumo-google[\s\S]*addEventListener\("click", requestGoogleAccess\)/, "autorização deve começar por clique", "firebase-consumo.js");
if (/db\.collection\(|firebase\.firestore\(|localStorage\.setItem\([^,]*token/i.test(firebaseGauge)) fail("firebase-consumo.js: medidor não pode ler Firestore nem salvar o token");
